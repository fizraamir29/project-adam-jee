  ...
  return (
    <div className="pt-32 pb-24 min-h-screen bg-[#fafbfc]">
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        {/* Featured Post */}
        <div className="mb-16">
          <Link to={`/blog/${BLOG_POSTS[0].id}`} className="group relative block rounded-3xl overflow-hidden bg-white border border-[#e2e8f0] shadow-sm hover:shadow-xl transition-all h-[500px]">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {BLOG_POSTS.slice(1).map(post => (
            <Link key={post.id} to={`/blog/${post.id}`} className="bg-white rounded-3xl border border-[#e2e8f0] shadow-sm hover:shadow-xl transition-all group overflow-hidden flex flex-col">
