              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredProducts.map(product => (
                  <div key={product.id} className="bg-white rounded-3xl p-5 border border-[#e2e8f0] shadow-sm hover:shadow-xl transition-all group flex flex-col h-full">
                    <Link to={`/product/${product.id}`} className="block relative aspect-square mb-6 overflow-hidden rounded-2xl bg-[#f8fafc]">
                    ...
                    <div className="flex-1 flex flex-col">
                      ...
                      <Link to={`/product/${product.id}`} className="text-lg font-bold text-[#0a1b2d] mb-2 hover:text-[#164475] transition-colors line-clamp-2">
                        {product.name}
                      </Link>
